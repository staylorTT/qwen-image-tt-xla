...
Optimizations over generate_image_v2.py:
  - Unbatched CFG: cond+uncond run as separate batch_size=1 passes (fits in DRAM at high res)
...
# SDPA fusing needs opt level 1 for the DevicePassesWrapper context.
# Opt level 1 works at 256/512 but may hang at 768+.
try:
...
def manual_attention(q, k, v):
    """Decomposed SDPA — fallback when fused op can't be used."""
...
def fused_attention(q, k, v):
    """Fused SDPA via tt-xla custom op → FlashAttention-2 on device.
    Requires seq_len divisible by 32.
    """
    from tt_torch.custom_ops import scaled_dot_product_attention as tt_sdpa
    scale = 1.0 / math.sqrt(q.shape[-1])
    return tt_sdpa(q, k, v, is_causal=False, scale=scale)
...
class QwenImageDemo:
    def __init__(self, weights_dir, width=256, height=256, num_steps=20, cfg_scale=4.0):
        ...
        # Compile each block individually — each block produces ONE graph
        # where the SDPA fusing pattern can match (matmul→softmax→matmul).
        # torch.compile on the full forward breaks into 300+ tiny graphs
        # that prevent fusing.
        ...
        print(f"  {len(self.compiled_blocks)} blocks compiled")

        # Pre-compute scheduler sigmas
        self._precompute_scheduler()

        # Warmup: compile batch_size=1 graphs (unbatched CFG uses same graph)
        print("Warming up (compiling graphs)...")
        t0 = time.perf_counter()
        self._generate_image("warmup", seed=0)  # CFG path (2x batch=1 passes)
    ...
    def _generate_image(self, prompt, negative_prompt="", seed=None):
        ...
        if do_cfg:
            cond_embeds_d = prompt_embeds.to(self.device)
            cond_masks_d = prompt_mask.to(torch.bfloat16).to(self.device)
            # Defer uncond embeddings — will move to device after cond pass frees memory
        else:
        ...
        for i in range(len(self.timesteps)):
            ...
            with torch.no_grad():
                if do_cfg:
                    # Run cond and uncond separately (unbatched) to halve peak DRAM
                    def _run_transformer(lat, emb, msk, t):
                        hs = self.transformer.img_in(lat)
                        ehs = self.transformer.txt_in(self.transformer.txt_norm(emb))
                        temb = self.transformer.time_text_embed(t.to(hs.dtype), hs)
                        for cb in self.compiled_blocks:
                            ehs, hs = cb(hs, ehs, msk, temb, self.rope_dev)
                        hs = self.transformer.norm_out(hs, temb)
                        return self.transformer.proj_out(hs)

                    noise_cond = _run_transformer(
                        latents_d, cond_embeds_d, cond_masks_d, ts)
                    torch_xla.sync()  # free cond intermediates before uncond pass
                    # Move uncond embeddings to device after cond pass frees memory
                    uncond_embeds_d = neg_embeds.to(self.device)
                    uncond_masks_d = neg_mask.to(torch.bfloat16).to(self.device)
                    noise_uncond = _run_transformer(
                        latents_d, uncond_embeds_d, uncond_masks_d, ts)
